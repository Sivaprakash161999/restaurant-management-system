...
def system():
    ...
    # defining the database connectivity
    def Database():
    ...
    def add():
        ...
        if (orders == "" or pizzas == '' or burgers == "" or drinkss == "" or
                ices == "" or costs == "" or subtotals == "" or taxs == "" or
                services == "" or totals == ""):
            messagebox.showinfo("Warning", "Please fill the empty field!!!")
        else:
            connectn.execute(
            '''INSERT INTO Restaurantrecords
            (ordno, piz, bur, ice, dr, ct, sb, tax, sr, tot)
            VALUES (?,?,?,?,?,?,?,?,?,?)''',
            (orders, pizzas, burgers, ices, drinkss,
                costs, subtotals, taxs, services, totals)
            )
            connectn.commit()
    ...
    # Top part
    main_lbl = Label(topframe, font=('Calibri', 25, 'bold'), text="Restaurant Management System",
                fg='Blue', anchor=W)
    ...
    ### Labels
    # items
    ordlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Order No.", fg='black',
                bd=5, anchor=W).grid(row=1, column=0)
    ordtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4,
                justify='right', textvariable=orderno).grid(row=1, column=1)

    # Pizza
    pizlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Pizza", fg='black',
                bd=5, anchor=W).grid(row=2, column=0)
    piztxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4,
                justify='right', textvariable=pizza).grid(row=2, column=1)

    # burger
    burlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Burger", fg='black',
                bd=5, anchor=W).grid(row=3, column=0)
    burtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4,
                justify='right', textvariable=burger).grid(row=3, column=1)

    # icecream
    icelbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Ice Cream", fg='black',
                bd=5, anchor=W).grid(row=4, column=0)
    icetxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4,
                justify='right', textvariable=icecream).grid(row=4, column=1)

    # drinks
    drinklbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Drinks", fg='black',
                bd=5, anchor=W).grid(row=5, column=0)
    drinktxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4,
                justify='right', textvariable=drinks).grid(row=5, column=1)

    # cost
    costlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Cost", fg='black',
                bd=5, anchor=W).grid(row=6, column=0)
    costtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4,
                justify='right', textvariable=cost).grid(row=6, column=1)

    # subtotal
    sublbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Subtotal", fg='black',
                bd=5, anchor=W).grid(row=7, column=0)
    subtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4,
                justify='right', textvariable=subtotal).grid(row=7, column=1)

    # tax
    taxlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Tax", fg='black',
                bd=5, anchor=W).grid(row=8, column=0)
    taxtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4,
                justify='right', textvariable=tax).grid(row=8, column=1)

    # service
    servicelbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Service", fg='black',
                bd=5, anchor=W).grid(row=9, column=0)
    servicetxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4,
                justify='right', textvariable=service).grid(row=9, column=1)

    # total
    totallbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Total", fg='black',
                bd=5, anchor=W).grid(row=10, column=0)
    totaltxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4,
                justify='right', textvariable=total).grid(row=10, column=1)

    # --button--

    totbtn = Button(leftframe, font=('Calibri', 16, 'bold'), text='Total', bg='Lightgrey',
                    fg='black', bd=3, padx=5, pady=5, width=6, command=tottal).grid(row=6, column=3)
    resetbtn = Button(leftframe, font=('Calibri', 16, 'bold'), text='Reset', bg='Lightgrey',
                    fg='black', bd=3, padx=5, pady=5, width=6, command=reset).grid(row=4, column=3)
    exitbtn = Button(leftframe, font=('Calibri', 16, 'bold'), text='Exit The System', bg='Lightgrey',
                    fg='black', bd=3, padx=5, pady=5, width=12, command=exit).grid(row=6, column=2)
    addbtn = Button(leftframe, font=('Calibri', 16, 'bold'), text='Add', bg='Lightgrey',
                    fg='black', bd=3, padx=5, pady=5, width=6, command=add).grid(row=2, column=3)
    deletebtn = Button(leftframe, font=('Calibri', 16, 'bold'), text='Delete Record', bg='Lightgrey',
                    fg='black', bd=3, padx=5, pady=5, width=12, command=Delete).grid(row=4, column=2)
    ...
    def feedbackk():
        ...
        def submit():
            ...
            cursor = conn.cursor()
            sql = "INSERT INTO FEEDBACK (n, eid, feedback5, com) VALUES ('{}', '{}', '{}', '{}')".format(n, eid, com, feedback5)
            # print(sql)
            cursor.execute("INSERT INTO FEEDBACK (n, eid, feedback5, com) VALUES (?,?,?,?)", (n, eid, feedback5, com))
            conn.commit()
            conn.close()
        ...
        # label #
        lb1 = Label(feed, font=('Calisto MT', 15, 'bold'), text="Thanks for Visiting!",
                    fg='black').pack(side=TOP)
        lbl2 = Label(feed, font=('Calisto MT', 15, 'bold'), text="We're glad you chose us! Please tell us how it was!",
                    fg='black').pack(side=TOP)

        # name
        namelbl = Label(feed, font=('Vardana', 15), text="Name:-", fg='black',
                        bd=10, anchor=W)
        namelbl.place(x=10, y=150)
        nametxt = Entry(feed, font=('Vardana', 15), bd=6, insertwidth=2, bg='white',
                        justify='right', textvariable=name)
        nametxt.place(x=15, y=185)
        # email
        emaillbl = Label(feed, font=('Vardana', 15), text="Email:-", fg='black',
                        bd=10, anchor=W)
        emaillbl.place(x=280, y=150)
        emailtxt = Entry(feed, font=('Vardana', 15), bd=6, insertwidth=2, bg='white',
                        justify='right', textvariable=email)
        emailtxt.place(x=285, y=185)
        # checkbutton
        ratelbl = Label(feed, font=('Vardana', 15), text="How would you rate us?", fg='black',
                        bd=10, anchor=W).place(x=10, y=215)
        checkvar1 = StringVar()
        checkvar2 = StringVar()
        checkvar3 = StringVar()
        checkvar4 = StringVar()
        ...
        # comments #
        commentslbl = Label(feed, font=('Calibri', 15), text="Comments", fg='black', bd=10,
                            anchor=W).place(x=10, y=300)
        txt = Text(feed, width=50, height=5)
        txt.place(x=15, y=335)
        # button
        submit = Button(feed, font=('Calibri', 15), text="Submit", fg='black', bg='green',
                        bd=2, command=submit).place(x=145, y=430)
        cancel = Button(feed, font=('Calibri', 15), text="Cancel", fg='black', bg='green',
                        bd=2, command=cancel).place(x=245, y=430)
        feed.mainloop()

    # Feedback Button
    feedbtn = Button(leftframe, font=('Calibri', 16, 'bold'), text="Feedback Form", fg='black',
                        bg='lightgrey', bd=3, padx=10, pady=10, width=10, command=feedbackk).grid(row=8, column=2, columnspan=1)
    ...
    def menu():
        ...
        roott.mainloop()
    # menubutton
    menubtn = Button(leftframe, font=('Calibri', 16, 'bold'), text="Menu Card", fg='black',
                        bg='lightgrey', bd=3, padx=6, pady=6, width=12, command=menu).grid(row=2, column=2)
    root.mainloop()
system()
